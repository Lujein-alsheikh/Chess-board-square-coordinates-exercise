...
# loading the images
def load_black_images(img_size = config.image_size):
...
def load_white_images(img_size = config.image_size):
...
def draw_board(screen= screen, board_left_top_x= config.board_left_top_x, board_left_top_y= config.board_left_top_y, board_length= config.board_length):
...
def draw_pieces(board_side, board_left_top_x= config.board_left_top_x, board_left_top_y=config.board_left_top_y, square_length = config.square_length):
...
def draw_button(color, rect, text, screen= screen):
    ...
    text_surface = config.font.render(text, True, 'white')
...
# buttons rectangles       
button_rect_find_square = button_rect(top_left_x = 30, top_left_y = config.highest_button_top_left_y, width = 170, height = 60)   
button_rect_find_coord = button_rect(top_left_x = 200, top_left_y = config.highest_button_top_left_y, width = 170, height = 60 )
button_rect_white_sided_board = button_rect(top_left_x = 30, top_left_y = config.highest_button_top_left_y + 60 + 16, width = 113.33, height = 60)
button_rect_randomly_sided_board = button_rect(top_left_x = 143.33, top_left_y = config.highest_button_top_left_y + 60 + 16, width = 113.33, height = 60)
button_rect_black_sided_board = button_rect(top_left_x = 256.66, top_left_y = config.highest_button_top_left_y + 60 + 16, width = 113.33, height = 60)
start_button_rect = button_rect(top_left_x = 30, top_left_y = config.highest_button_top_left_y + 2*(60 + 16), width = 340, height = 60)

def draw_all_buttons(square_button_color, coord_button_color, white_sided_board_color, randomly_sided_board_color, black_sided_board_color, start_button_color):
    ...
    draw_button(start_button_color, start_button_rect, "START")
...
def click_within_square(click_x, click_y, square_top_left_x, square_top_left_y, square_length = config.square_length):
...
def click_within_board(click_x, click_y, board_left_top_x = config.board_left_top_x, board_left_top_y= config.board_left_top_y, board_length=  config.board_length):
      if board_left_top_x <= click_x <= board_left_top_x + board_length and board_left_top_y <= click_y <= board_left_top_y + board_length:
            return True
      else:
            return False        

def draw_coord(coord, screen= screen,  board_left_top_x = config.board_left_top_x, board_left_top_y= config.board_left_top_y, board_length= config.board_length):
                text_surface = config.big_font.render(coord, True, 'lavenderblush')
...
def finding_wrong_square(board_side, click_x, click_y, square_length = config.square_length):
...
def frame_around_square(board_side, click_x, click_y, square_length = config.square_length, frame_thickness = 2, color='darkred', board_left_top_x =config.board_left_top_x,
board_left_top_y = config.board_left_top_y):
...
def draw_score(some_score, color= 'olive', frame_thickness = 2, frame_top_left_x = 30, frame_top_left_y= config.highest_button_top_left_y + 3*(60+16), width= 340, height= 120):
        ...
        score_text =  config.font.render(f'Score: {some_score}', True, 'black')
        text_rect = score_text.get_rect(center=(frame_top_left_x + width / 2, frame_top_left_y + height / 2))
        screen.blit(score_text, text_rect)
        
def draw_timer(some_time, color= 'olive', frame_thickness=2,frame_top_left_x = 30, frame_top_left_y= config.highest_button_top_left_y + 3*(60+16) + 120+16, width= 340, height= 120):
                    inner_rect = pygame.Rect(frame_top_left_x , frame_top_left_y, width, height)
                    screen.fill(color, inner_rect)
                    ...
                    timer_text = config.font.render(f'Time left: {some_time:.1f}', True, 'black')
...
time_running_find_square = False
click = False
...
while run:
    ...
    for event in pygame.event.get(): 
        if event.type == pygame.QUIT: 
            ...
            if button_clicked(button_rect_find_square, event.pos):
                        ...
                        time_running_find_square = True
                        board_side = 'white'

                        random_coord = create_random_coord()
                        print("Initial Random coordinates:", random_coord)   
            ...
            elif find_square_button_clicked:
                    click = False
                    if click_within_board(click_x, click_y) == True:

                        if click_within_square(click_x =click_x, click_y = click_y, square_top_left_x =correct_square_top_left_x , square_top_left_y = correct_square_top_left_y, square_length = config.square_length):
                                        correct_square = True
                                        print(f"click within square {correct_square}")
    ...
    draw_all_buttons(square_button_color, coord_button_color, white_sided_board_color, randomly_sided_board_color, black_sided_board_color, start_button_color)
    
    
    if time_running_find_square:
...
''' # timer using pygame.time.get_ticks() primitive code! 
