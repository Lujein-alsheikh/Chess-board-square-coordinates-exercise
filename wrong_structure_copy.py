...
while run:
    ...
    for event in pygame.event.get(): 
        # print("inside the general event listener")
        if event.type == pygame.QUIT:  # if the user hits the x button.
            run = False
        
        elif event.type == pygame.MOUSEBUTTONDOWN:
            click_x, click_y = pygame.mouse.get_pos()
            # print("mouse click general loop", click_x, click_y)
...
'''' # When clicking the find square button, instead of the timer, we can use a for loop. 
This doesn't work perfectly because
when i=3, you can make one click and whether it is correct or not the code stops.
    i=0
    correct_square = True
    while i<= 2:
        same code for the timer
'''
''' # buttons defined explicitely
# Find square button rectangle
def button_rect_find_square():
        top_left_x = 30
        top_left_y = 184
        width = 170
        height = 60
        button_rect = pygame.Rect(top_left_x, top_left_y, width, height)
        return button_rect 

# Find coordinates button rectangle
def button_rect_find_coord():
        top_left_x = 200
        top_left_y = 184
        width = 170 
        height = 60 
