...
def click_on_find_or_name_square_buttons(click_pos):
       if find_square_button.button_clicked_fun(click_pos) or name_square_button.button_clicked_fun(click_pos):
              return True
       else:
              return False

def click_on_one_of_the_board_side_buttons(click_pos):
        if white_board_button.button_clicked_fun(click_pos) or black_board_button.button_clicked_fun(click_pos) or \
        random_board_button.button_clicked_fun(click_pos):
               return True
        else:
               return False
...
board = Board() 
pieces = Pieces()
find_square_button = Button(text= 'Find Square', color='firebrick',is_clicked=True, top_left_x=30, top_left_y = config.highest_button_top_left_y, width = 170, height = 60)
name_square_button = Button(text='Name Square', is_clicked=False, top_left_x = 200, top_left_y = config.highest_button_top_left_y, width = 170, height = 60)
white_board_button= Button(text='white', color='firebrick',is_clicked=True,top_left_x = 30, top_left_y = config.highest_button_top_left_y + 60 + 16, width = 113.33, height = 60)
random_board_button= Button("random",is_clicked=False,top_left_x = 143.33, top_left_y = config.highest_button_top_left_y + 60 + 16, width = 113.33, height = 60)
black_board_button=Button("black", is_clicked=False,top_left_x = 256.66, top_left_y = config.highest_button_top_left_y + 60 + 16,width = 113.33, height = 60)
start_button= Button("START", color='firebrick',is_clicked=False,top_left_x = 30, top_left_y = config.highest_button_top_left_y + 2*(60 + 16), width = 340,height = 60)
    ...
    for event in pygame.event.get(): 
        if event.type == pygame.QUIT: 
            ...
            if start_button.button_clicked_fun(event.pos):
                    if start_button.get_clicked(): # if a game is already in play; don't allow for a new game to start.
                          pass
                    else:
                ...
                if start_button.get_clicked() == False: # if the start button has not been already clicked on
                    if click_on_find_or_name_square_buttons(event.pos):
                            ...
                            if white_board_button.button_clicked_fun(event.pos):
                                ...
                                black_board_button.unclick()
                                board.set_random_side(True) # an initial random side is chosen here.
            ...
            if name_square_button.get_clicked() and start_button.get_clicked() and game_state.get_allowed_to_read_input():
                
                if event.key == pygame.K_RETURN:
                    name_square_response.set_input("")
                elif event.key == pygame.K_BACKSPACE:
                    name_square_response.set_input(name_square_response.get_input()[:-1])

                else:
                    if len(name_square_response.get_input()) < 2: # limit the input to 2 characters
                        name_square_response.set_input(name_square_response.get_input() + event.unicode)
 
                print(f"input is {name_square_response.get_input()} of length {len(name_square_response.input_answer)}")
            ...
            if game_state.get_find_square_time_running():
                if game_state.get_click_find_square() is False:    
                    ...
                    pygame.display.update()
                        ...
                        if find_square_response.get_correct_square_answered(): # if correct_square was guessed

                                    find_square_response.draw_frame_around_square(screen, board.get_board_side())
                                    pygame.display.flip()
                                    pygame.time.wait(50) # wait for 50 ms
                                    # The previous three instructions are to draw a green frame around a correctly guessed square
                                    board.draw(screen)
                                    print("---------------------------------------------")
